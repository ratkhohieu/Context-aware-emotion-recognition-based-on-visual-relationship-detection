...
def main():
    ...
    root = './emotic'
    train_dataset = Emotic_Dataset_places(train_context, train_body, train_cat, train_cont, train_transform,
                                          mode_unnorm=False,
                                          hook_faster=load_data_hook_faster('train'),
                                          hook_sentiment=load_data_sentiment('train'),
                                          hook_places=load_data_places('train'))
    ...
    test_dataset = Emotic_Dataset_places(test_context, test_body, test_cat, test_cont, test_transform,
                                         mode_unnorm=False,
                                         hook_faster=load_data_hook_faster('test'),
                                         hook_sentiment=load_data_sentiment('test'),
                                         hook_places=load_data_places('test'))
    ...
    model_hook = Objects_net(256, 4, 50, N=10, num_sentiment=4096, num_body=2048, embracement_size=128)
    # model = Emotic_v4(num_context_features=2048, num_body_features=2048, num_hook=128)
    model = Emotic_v6(list(model_context.children())[-1].in_features,
                      list(model_body.children())[-1].in_features,
                      num_hook=128, num_places=50)
    ...
    with trange(args.num_epochs, total=args.num_epochs, desc='Epoch') as t:
        for epoch in t:
            if optimizer.param_groups[0]['lr'] < 5e-4:
                unfreeze(model_context, percent=0.5)
                unfreeze(model_body, percent=0.5)
            ...
            for batch_idx, sample in tqdm(enumerate(train_loader), total=len(train_loader)):
                ...
                places = sample['places_attribute'][:, -1, :].to(device).float()
                ...
                pred_hook = model_hook(features_object, bbox, word_embedding, sentiment_object, pred_body)
                pred_cat, pred_context, pred_body, pred_hook = model(pred_context, pred_body, pred_hook, places)
            ...
            with torch.no_grad():
                ...
                for batch_idx, sample in tqdm(enumerate(test_loader), total=len(test_loader), desc='Test_mode'):
                    ...
                    pred_hook = model_hook(features_object, bbox, word_embedding, sentiment_object, pred_body)
                    places = sample['places_attribute'][:, -1, :].to(device).float()
                    pred_cat, _, _, _ = model(pred_context, pred_body, pred_hook, places)
                    # pred_cat = model(places)
                    pred_cat = pred_cat.sigmoid()
