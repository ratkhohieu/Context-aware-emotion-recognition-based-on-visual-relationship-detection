...
def main():
    ...
    model_hook = Sentiment_MLP(256, 4, 50, N=10, num_sentiment=4096, embracement_size=52)
    model = Emotic_v5(list(model_context.children())[-1].in_features,
    ...
    with trange(args.num_epochs, total=args.num_epochs, desc='Epoch') as t:
        for epoch in t:
            ...
            for batch_idx, sample in tqdm(enumerate(train_loader), total=len(train_loader)):
                ...
                pred_cont, pred_context, pred_body, pred_hook = model(pred_context, pred_body, pred_hook)
                # pred_cat = model(pred_context, pred_body, pred_hook)
                # pred_cat = pred_cat.sigmoid()

                with torch.cuda.amp.autocast():
                    cat_loss_batch = criterion2(pred_cont, labels_cont)

                    cat_loss_context = criterion2(pred_context, labels_cont)
                    cat_loss_body = criterion2(pred_body, labels_cont)
                    cat_loss_hook = criterion2(pred_hook, labels_cont)

                    # cont_loss_batch = criterion2(pred_cont * 10, labels_cont * 10)
                    loss = cat_loss_batch + 0.1*(cat_loss_context + cat_loss_body + cat_loss_hook)
            ...
            with torch.no_grad():
                ...
                for batch_idx, sample in tqdm(enumerate(test_loader), total=len(test_loader), desc='Test_mode'):
                    ...
                    # labels_cat = sample['cat_label'].to(device).float()
                    labels_cont = sample['cont_label'].to(device).float()
