...
def main():
    ...
    for mode_list in [0]:
        ...
        test_loader = DataLoader(dataset=test_dataset,
                                 ...
                                 num_workers=4,
                                 shuffle=False)
        # model = CaerNet_v3(num_context_features=2048, num_body_features=2048, num_hook=256,
        #                    num_box=4096, weight=True, mode_list=mode_list)

        model = torch.load(f'./weight/checkpoints/stream4_{mode_list}_caer_model.pth')
            ...
            for epoch in t:
                ...
                with torch.no_grad():
                    ...
                    if (correct / total) > best_scores:
                        ...
                        np.save(f'./weight/results/{mode_list}_caer_ablation.npy', cat_preds)
                        os.makedirs('./weight/checkpoints', exist_ok=True)
                        # torch.save(model, f'./weight/checkpoints/stream4_caer_model.pth')
                        torch.save(model, f'./weight/checkpoints/stream4_{mode_list}_caer_model.pth')
