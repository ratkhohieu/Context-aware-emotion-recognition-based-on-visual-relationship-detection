...
parser.add_argument('--num_epochs', default=15, type=int, help='number epochs')
...
def main():
    ...
    train_dataset = Emotic_CSVDataset(cat2ind, train_transform, mode_cat=None,
    ...
    test_dataset = Emotic_CSVDataset(cat2ind, test_transform, mode_cat=None,
    ...
    model = Objects_netv2(1024, 28, 50, N=10, num_sentiment=4096, num_body=2048,
                          embracement_size=128)
    # _, model_body = prep_models(model_dir='proj/debug_exp/models', context_model='resnet50')
    #
    ...
    with trange(args.num_epochs, total=args.num_epochs, desc='Epoch') as t:
        for epoch in t:
            ...
            for batch_idx, sample in tqdm(enumerate(train_loader), total=len(train_loader)):
                ...
                pred_cat = model(features_object, bbox,
