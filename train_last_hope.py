...
def main():
    ...
    train_dataset = Emotic_CSVDatasetv2(cat2ind, train_transform, mode_cat=None,
                                        data_src='emotic_pre/train.csv', root=root,
    ...
    test_dataset = Emotic_CSVDatasetv2(cat2ind, test_transform, mode_cat=None,
                                       data_src='emotic_pre/test.csv', root=root,
    ...
    best_scores = 0
    # import pdb; pdb.set_trace()
    # for name, p in model.module.model_object.named_parameters():
    ...
    with trange(args.num_epochs, total=args.num_epochs, desc='Epoch') as t:
        for epoch in t:
            if optimizer.param_groups[0]['lr'] < 5e-4:
                unfreeze(model.module.model_object, percent=0.3)
                unfreeze(model.module.model_body, percent=0.3)
                unfreeze(model.module.model_context, percent=0.3)
            ...
            for batch_idx, sample in tqdm(enumerate(train_loader), total=len(train_loader)):
                ...
                pair_o_m = sample['pair_o_m'].to(device).float()
                # import pdb; pdb.set_trace()
                pair_o_m = pair_o_m.permute(0, 1, 4, 2, 3).reshape(-1, 30, 320, 320)
                ...
                pred_cat = model(images_context, images_body, pair_o_m)
