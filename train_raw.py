...
def main():
    ...
    for fold in range(5):
        ...
        test_dataset = Emotic_Dataset_addhookv2(test_context, test_body, test_cat, test_cont, test_transform,
                                                data_face='./emotic/emotic_pre/test_face.csv',
                                                index_train=None,
        ...
        model = Emotic_v10(num_context_features=2048, num_body_features=2048, num_hook=32, cont_vad=False,
                           embracement_size=32, num_box=4, num_face=32)
        ...
        with trange(args.num_epochs, total=args.num_epochs, desc='Epoch') as t:
            for epoch in t:
                if optimizer.param_groups[0]['lr'] < 5e-4:
                    unfreeze(model.module.model_context, percent=0.5)
                    unfreeze(model.module.model_body, percent=0.5)
                ...
                for batch_idx, sample in tqdm(enumerate(train_loader), total=len(train_loader)):
                    ...
                    pred_cat, context, body, hook = model(images_context, images_body, features_object, bbox,
                                                          word_embedding, sentiment_object, images_face)
                ...
                with torch.no_grad():
                    ...
                    for batch_idx, sample in tqdm(enumerate(test_loader), total=len(test_loader), desc='Test_mode'):
                        ...
                        # places = sample['places_attribute'][:, -1, :].to(device).float()
                        pred_cat, _, _, _ = model(images_context, images_body, features_object, bbox, word_embedding,
                                                  sentiment_object, images_face)
