...
parser.add_argument('--num_epochs', default=40, type=int, help='number epochs')
parser.add_argument('--alpha', default=1., type=float, help='number alpha')
        ...
        with trange(args.num_epochs, total=args.num_epochs, desc='Epoch') as t:
            for epoch in t:
                if optimizer.param_groups[0]['lr'] < 5e-4:
                    unfreeze(model.module.model_context, percent=0.2)
                    unfreeze(model.module.model_body, percent=0.2)
                ...
                for batch_idx, sample in tqdm(enumerate(train_loader), total=len(train_loader)):
                    ...
                    inputs, targets_a, targets_b, lam = mixup_data(images_context, labels_cat, args.alpha)

                    pred_cat, context, body, hook = model(inputs, images_body, features_object, sp_box,
                    ...
                    with torch.cuda.amp.autocast():
                        ...
                        loss_context = mixup_criterion(criterion1, pred_cat, targets_a, targets_b, lam)
