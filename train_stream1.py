...
def main():
    ...
    test_loader = DataLoader(dataset=test_dataset,
                             ...
                             num_workers=4,
                             shuffle=False)
    model = Caer_stream1(num_context_features=2048, num_body_features=2048, num_hook=256,
                         num_box=4096, weight=True)

    # model = torch.load(f'./weight/checkpoints/{fold}_model.pth')
    # import pdb; pdb.set_trace()
    ...
    with trange(args.num_epochs, total=args.num_epochs, desc='Epoch') as t:
        for epoch in t:
            ...
            for batch_idx, sample in tqdm(enumerate(train_loader), total=len(train_loader)):
                ...
                pred_cat = model(images_body, image_face)
        ...
        with torch.no_grad():
            ...
            if correct / total > best_scores:
                ...
                np.save(f'./weight/results/stream1_caer.npy', cat_preds)
                ...
                torch.save(model, f'./weight/checkpoints/stream1_caer_model.pth')
