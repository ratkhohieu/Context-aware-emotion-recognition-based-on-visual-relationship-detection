...
def main():
    ...
    root = './emotic'
    train_dataset = Emotic_Mask_Dataset(train_context, train_body, train_cat, train_cont, train_transform,
                                        root=root,
    ...
    valid_dataset = Emotic_Mask_Dataset(val_context, val_body, val_cat, val_cont, test_transform,
                                        root=root,
    ...
    test_dataset = Emotic_Mask_Dataset(test_context, test_body, test_cat, test_cont, test_transform,
                                       root=root,
    ...
    model = Emotic_mask(list(model_context.children())[-1].in_features,
                        list(model_body.children())[-1].in_features,
                        list(model_face.children())[-1].in_features)
    ...
    optimizer = torch.optim.SGD(
        (list(model.parameters()) + list(model_context.parameters())
         + list(model_body.parameters())) + list(model_face.parameters()),
    ...
    with trange(args.num_epochs, total=args.num_epochs, desc='Epoch') as t:
        for epoch in t:
            ...
            for batch_idx, sample in tqdm(enumerate(train_loader), total=len(train_loader)):
                images_context = sample['image_mask'].to(device)
                ...
                pred_cat, pred_cont = model(pred_context, pred_body, pred_face)
