...
def prepare_data(data_mat, data_path_src, save_dir, dataset_type='train', generate_npy=False):
        ...
        for person in range(nop):
            ...
            try:
                ...
                if not os.path.exists(image_path):
                    ...
                    # context[..., ::-1][et.bbox[1]:et.bbox[3], et.bbox[0]:et.bbox[2]] = 0
                    # import pdb; pdb.set_trace()
                    context_cv = cv2.resize(context, (256, 256))
                    body_cv = cv2.resize(body, (256, 256))
    ...
    if generate_npy == True:
        ...
        np.save(os.path.join(save_dir, '%s_context_256.npy' % (dataset_type)), context_arr)
        # np.save(os.path.join(save_dir, '%s_body_256.npy' % (dataset_type)), body_arr)
        # np.save(os.path.join(save_dir, '%s_cat_256.npy' % (dataset_type)), cat_arr)
        # np.save(os.path.join(save_dir, '%s_cont_256.npy' % (dataset_type)), cont_arr)
...
def parse_args():
    ...
    parser.add_argument('--label', type=str, default='val', choices=['train', 'val', 'test', 'all'])
...
if __name__ == '__main__':
    args = parse_args()
    ann_path_src = os.path.join(args.data_dir, 'emotic/Annotations', 'Annotations.mat')
